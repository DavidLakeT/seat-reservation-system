...
impl Seat {
    ...
    pub fn set_on_payment(&mut self, username: String) -> Result<(), Error> {
        ...
        if matches!(self.status, SeatStatus::OnPayment) || matches!(self.status, SeatStatus::Reserved) {

            return Err(Error::new(ErrorKind::Other, "Seat already waiting for payment or reserved."));
        }
    ...
    pub fn set_canceled(&mut self, username: String) -> Result<(), Error> {
        ...
        if let Some(reservation_username) = self.get_reservation_username() {
            ...
            if username.eq_ignore_ascii_case(&reservation_username) {
                ...
                Ok(())
            } else { Err(Error::new(ErrorKind::Other, "El usuario no corresponde con la reserva.")) }
        } else { Err(Error::new(ErrorKind::Other, "Seat is not reserved.")) }
    }
    ...
    pub fn set_reserved(&mut self, username: String) -> Result<(), Error> {
        ...
        } else { Err(Error::new(ErrorKind::Other, "Seat was not waiting for payment.")) }
    }
