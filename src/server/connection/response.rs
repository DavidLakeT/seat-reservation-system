...
impl Response {
    ...
    pub fn get_response(request: &MenuSelection, seats: Vec<Seat>) -> (Self, Vec<Seat>) {
        ...
        match request {
            ...
            MenuSelection::DisplayReservedSeats(_username) => { Self::create_display_response(request, seats) },
            MenuSelection::ReserveSeat(username, seat_id) => { Self::create_reserve_response(seats, username.clone(), *seat_id) },
            MenuSelection::CancelReservation(username, seat_id) => { Self::create_cancel_response(seats, username.clone(), *seat_id) },
            MenuSelection::Checkout(username, seat_id) => { Self::create_checkout_response(seats, username.clone(), *seat_id) },
        }
    }
    ...
    fn create_reserve_response(mut seats: Vec<Seat>, username: String, seat_id: i64) -> (Self, Vec<Seat>){
    ...
    fn create_cancel_response(mut seats: Vec<Seat>, username: String, seat_id: i64) -> (Self, Vec<Seat>) {
    ...
    fn create_checkout_response(mut seats: Vec<Seat>, username: String, seat_id: i64) -> (Self, Vec<Seat>) {
